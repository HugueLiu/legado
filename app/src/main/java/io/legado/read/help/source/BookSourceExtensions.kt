...
import io.legado.read.data.entities.rule.ExploreKind
...
private fun BookSource.getExploreKindsKey(): String {
    return io.legado.read.utils.MD5Utils.md5Encode(bookSourceUrl + exploreUrl)
}

suspend fun BookSource.exploreKinds(): List<ExploreKind> {
    val exploreKindsKey = getExploreKindsKey()
    io.legado.read.help.source.exploreKindsMap[exploreKindsKey]?.let { return it }
    val exploreUrl = exploreUrl
    if (exploreUrl.isNullOrBlank()) {
        return emptyList()
    }
    val mutex = io.legado.read.help.source.mutexMap[bookSourceUrl] ?: Mutex().apply { io.legado.read.help.source.mutexMap[bookSourceUrl] = this }
    mutex.withLock {
        io.legado.read.help.source.exploreKindsMap[exploreKindsKey]?.let { return it }
        ...
        withContext(Dispatchers.IO) {
            kotlin.runCatching {
                ...
                if (exploreUrl.startsWith("<js>", true)
                    ...
                    ruleStr = io.legado.read.help.source.aCache.getAsString(exploreKindsKey)
                    if (ruleStr.isNullOrBlank()) {
                        ...
                        io.legado.read.help.source.aCache.put(exploreKindsKey, ruleStr)
                    }
                }
                if (ruleStr.isJsonArray()) {
                    io.legado.read.utils.GSON.fromJsonArray<ExploreKind?>(ruleStr).getOrThrow()?.let {
        ...
        io.legado.read.help.source.exploreKindsMap[exploreKindsKey] = kinds
        return kinds
    }
}
...
suspend fun BookSource.clearExploreKindsCache() {
    withContext(Dispatchers.IO) {
        val exploreKindsKey = getExploreKindsKey()
        io.legado.read.help.source.aCache.remove(exploreKindsKey)
        io.legado.read.help.source.exploreKindsMap.remove(getExploreKindsKey())
    }
}
