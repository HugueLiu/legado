...
import io.legado.read.help.MediaHelp
import io.legado.read.help.config.AppConfig
import io.legado.read.help.exoplayer.ExoPlayerHelper
...
import io.legado.read.receiver.MediaButtonReceiver
        ...
        durChapter?.let { chapter ->
            if (addLoading(chapter.index)) {
                ...
                if (book != null && bookSource != null) {
                    io.legado.read.model.webBook.WebBook.getContent(this@AudioPlayService, bookSource, book, chapter)
                        .onSuccess { content ->
                            if (content.isEmpty()) {
                                withContext(Main) {
                                    toastOnUi("未获取到资源链接")
